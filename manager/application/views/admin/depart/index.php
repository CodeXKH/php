...
<html>
	...
	<body>
			...
			<div id="layout_right_content" class="layout_right_content">
				...
				<!-- 部门列表导航条 -->			
				<div class="route_nav">
					<button type="button" class="btn btn-success nav_add">添加部门</button>
					<button type="button" class="btn btn-success nav_search">搜索部门</button>
					<input type="text" class="form-control" id="nav_txt" placeholder="请输入部门名称" value="<?=$name ?>">
					...
					<table class="table table-hover">
						...
						<?php foreach ($depart as $item):?>
							<tr style="text-align: center">
								...
								<td>
									<a href="<?php echo base_url().'admin/depart/main/edit/'.$item->id?>"  title="更新"><i class="fa fa-pencil-square-o edit_hover"></i></a>
		...
		<script type="text/javascript" language="javascript">
	        ...
	        $(document).ready(function() {
	            ...
	            $(".nav_add").click(function(){
						location.href="<?php echo base_url().'admin/depart/main/add'?>";
		         })
		         
		        //模糊查询
	            $(".nav_search").click(function(){
	            	var name=$('#nav_txt').val();
					if(isEmpty(name)){
						 $(".nav_search").focus();
						 alert("内容不能为空!");
						 return false;
	            	...
	            	location.href="<?php echo base_url().'admin/depart/main/index/'?>"+name;
		         })
	        });
	        ...
	        function td_delete(id){
				...
				$.tmDialog.confirm({width:450,height:250,title:"退出",icon:icon,content:"您确认删除吗!!!",callback:function(ok){
					if(ok){
			        	$.ajax({
							type:"post",
							url:"<?php echo site_url('admin/depart/main/depart_delete');?>",
							data:{"id":id},
	        ...
	        PageClick = function(pageclickednumber) {
				...
				$.ajax({
					type:"post",
					url:"<?php echo base_url().'admin/depart/main/depart_page'?>",
					dataType:"json",
					data:{"pagenumber":pageclickednumber,"name":$('#nav_txt').val()},
					success:function(data){
						var html="";
						...
						for(var n=0;n<data.length;n++){
							html+="<tr style='text-align: center'>"
								...
								+"  <td>"
								+"	  <a href='<?php echo base_url().'admin/depart/main/edit/'?>"+data[n].id+"' title='更新'><i class='fa fa-pencil-square-o'></i></a>"
